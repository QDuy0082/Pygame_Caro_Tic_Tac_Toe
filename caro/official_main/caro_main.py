...
class Gamestate():
	...
	def mid(self):
		...
		for i in range (0, til):
			DISPLAYSURF.blit(bg,(i*bg_width+scroll,0))
		drawboard()
		textsurface1 = textsurface('PLAYER 1')
	...
	def mid1(self):
		...
		for i in range (0, til):
			DISPLAYSURF.blit(bg,(i*bg_width+scroll,0))
		drawboard()
		textsurface1 = textsurface('PLAYER 2')
		textrect1 = textrect(textsurface1)
		DISPLAYSURF.blit(textsurface1,textrect1)
		button_back_bf_img.draw(DISPLAYSURF)
		button_next_bf_img.draw(DISPLAYSURF)
		...
		for event in pygame.event.get():
			...
			if event.type == KEYDOWN and (event.key == K_KP_ENTER or event.key==K_RETURN):
				...
				pygame.time.wait(200)
				self.state = 'main_game'
		TEXT_2.draw_main(RECT_TEXT)
		pygame.display.update()
		DISPLAYSURF.fill(BGCOLOR)
	def main_game(self):
		global mousex, mousey, count, win, temp2, arr_pos, clicked, text, text_rect, counter, flink
		mixer.music.stop()
		mouseClick = False
		...
		pygame.draw.rect(DISPLAYSURF, '#160043', (XMARGIN+22*(BOXSIZE+GAPSIZE)-5,YMARGIN+4*(BOXSIZE+GAPSIZE)-5,340,49))
		pygame.draw.rect(DISPLAYSURF, '#160043', (XMARGIN+22*(BOXSIZE+GAPSIZE)-5,YMARGIN+10*(BOXSIZE+GAPSIZE)-5,340,49))		
		TEXT_1.draw_main(RECT_TEXT_1)
		TEXT_2.draw_main(RECT_TEXT_2)
		if (count % 2) == 1:	
			if flink%2 == 1:
				TEXT_2.draw_main_1(RECT_TEXT_2)
		else:
			if flink % 2 ==1:
				TEXT_1.draw_main_1(RECT_TEXT_1)
			...
			if not markedBoxes[boxx][boxy] and mouseClick:
				if count >= 0:
					...
					if checkwin(mainboard,boxx,boxy,ALLSHAPES[count % 2]) == 1:
						...
						self.state = 'end'
					if count == BOARDHEIGHT*BOARDWIDTH-1:
						self.state = 'end'
				if counter > 0: 
					count += 1
			if counter == 0: 
	...
	def end(self):
		global scroll,win,mainboard,markedBoxes, exit, temp2, count, arr_pos, temp, currentFrame, counter
		DISPLAYSURF.blit(bg_end,(0,0))
		if temp2 == 1:
			if temp == 0: victory_sound.play(0)
			temp += 1
			...
			winner_font_2 = pygame.font.SysFont('Cambria Math',120,True)
			winner_text_2 = winner_font_2.render("Draw", True, BLACK)
			winner_text_2_rect = winner_text_2.get_rect(center=(WINDOWWIDTH/2,250))
			DISPLAYSURF.blit(winner_text_2,winner_text_2_rect)
...
class text():
	...
	def draw_main_1(self,pos_main):
		...
		pygame.draw.rect(DISPLAYSURF,'#FFEC68',self.rect_text,border_radius = 2)
...
TEXT_2 = text(150,56,(338,280),330)
RECT_TEXT = (338,280)
...
flinker_event = pygame.USEREVENT+2
pygame.time.set_timer(timer_event, 1000)
pygame.time.set_timer(flinker_event, 400)
