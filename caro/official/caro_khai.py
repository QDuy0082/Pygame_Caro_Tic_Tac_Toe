...
class Gamestate():
	...
	def main_game(self):
		...
		if counter > 0: drawArcCv2(DISPLAYSURF, color, (XMARGIN+22*(BOXSIZE+GAPSIZE)+160,YMARGIN+15*(BOXSIZE+GAPSIZE)+50), 60, 10, -360*counter/20)
		# TEXT_1.draw_main(RECT_TEXT_1)
		# TEXT_2.draw_main(RECT_TEXT_2)

		if count % 2 == 0  or count == -1:
			TEXT_1.animation_text(RECT_TEXT_1,count)
		if count % 2 == 1 and count >=1:
			TEXT_2.animation_text(RECT_TEXT_2,count)
	...
	def end(self):
		global scroll,win,mainboard,markedBoxes, exit, count, arr_pos, currentFrame
		# for i in range (0, til):
		...
		winner_font_2 = pygame.font.SysFont('Cambria Math',110,True)
		...
		winner_text_2_rect = winner_text_2.get_rect(center=(WINDOWWIDTH/2,300))
...
def drawImage(count,pos_main,rect_width):
	if count % 2 == 0  or count == -1:
		DISPLAYSURF.blit(x_image,(pos_main+int((rect_width-x_image.get_width())/2),10*(BOXSIZE+GAPSIZE)))
	if count % 2 == 1 and count >=1:
		DISPLAYSURF.blit(o_image,(pos_main+int((rect_width-x_image.get_width())/2),10*(BOXSIZE+GAPSIZE)))
...
class text():
	...
	def animation_text(self,pos_main, count):
		global ALLSHAPES
		self.rect_text.h = 250


		self.text_rect = self.text_surface.get_rect(center = self.rect_text.center)
		self.text_rect.center = self.rect_text.center
		if count % 2 == 0  or count == -1:
			self.rect_text.x = pos_main[0]+10
			self.rect_text.y = pos_main[1]+10
			pygame.draw.rect(DISPLAYSURF,GRAY,self.rect_text,border_radius = 12)
		...
		self.rect_text.w = WINDOWWIDTH - self.rect_text.x -50

		collection = [word.split(' ') for word in self.text.splitlines()]
		space = base_font.size(' ')[0]
		x = self.rect_text.x+20
		y = self.rect_text.y+20
		for lines in collection:
			for words in lines:
				word_surface = base_font.render(words, True, WHITE)
				word_width , word_height = word_surface.get_size()
				if x + word_width >= WINDOWWIDTH - 50:
					x = self.rect_text.x+20
					y += word_height
				DISPLAYSURF.blit(word_surface, (x,y))
				x += word_width + space
			x = self.rect_text.x+20
			y += word_height

		drawImage(count,self.rect_text.x,self.rect_text.w)

	def reset_text(self):
		self.rect_text.x = self.pos_original[0]
		self.rect_text.y = self.pos_original[1]
		self.rect_text.w = self.width_original
		self.rect_text.h = self.height_original
		self.text = ''
...
RECT_TEXT_1 = (XMARGIN+22*(BOXSIZE+GAPSIZE),YMARGIN+2*(BOXSIZE+GAPSIZE))
RECT_TEXT_2 = (XMARGIN+22*(BOXSIZE+GAPSIZE),YMARGIN+2*(BOXSIZE+GAPSIZE))
...
scale_image = 0.5
x_image = pygame.image.load('x_image.png').convert_alpha()
x_image = pygame.transform.scale(x_image, (int (x_image.get_width() * scale_image), int (x_image.get_height() * scale_image)))
o_image = pygame.image.load('o_image.png').convert_alpha()
o_image = pygame.transform.scale(o_image, (int (o_image.get_width() * 0.06), int (o_image.get_height() * 0.06)))
